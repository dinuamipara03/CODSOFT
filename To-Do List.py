...
if __name__ == "__main__":
    ...
    root = Tk()
    # Change the title
    root.title("To-Do List By Dinaxi")
    # Change the window size
    root.geometry("550x250")
    # no resize for both directions
    root.resizable(False, False)
    root.configure(bg="#b68967")
    ...
    lbl_task_show = Label(
        root, text="Enter task here:", font=("Helvetica", 15), bg="#b68967"
    )
    txt_input = Entry(root, width=25, bd="2", font="18")
    lb_tasks = Listbox(
        root, width=30, height=8, selectmode="SINGLE", relief=RIDGE, bd="4", font="14"
    )
    btn_add_task = Button(
        root, text="Add task", width=20, command=addTask, relief=RIDGE
    )
    btn_del_one = Button(root, text="Delete", width=20, relief=RIDGE, command=delOne)
    btn_del_all = Button(
        root, text="Delete all", width=20, relief=RIDGE, command=deleteAll
    )
    ...
    # Place geometry
    lbl_title.place(x=205, y=5)
    lbl_task_show.place(x=10, y=45)
